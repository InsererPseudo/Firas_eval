 #Création de la table de jeu
  ...
  def affiche_compteurs():   
      ...
      # Décompte des cases à traiter pour la fonction gagné    
  ...
  def affiche_nb_mines(nb_mines_voisines, col, lig):   
      ...
      if tab_j[col, lig] == "":   
          ...
          can.create_rectangle((col-1)*dim+gap+3,(pLig-1)*dim+gap+3, col*dim+gap-3,pLig*dim+gap-3,width=0, fill="ivory")
          # Affichage du nombre de mines avec les couleurs corespondantes     
          coul = ['blue','orange','red','green','cyan','skyblue','pink']   
          can.create_text(col*dim-dim//2+gap, pLig*dim-dim//2+gap, text=str(nb_mines_voisines), fill=coul[nb_mines_voisines-1],font='Arial 22')
...
def vide_plage_zero(col, ligne): 
    ...
    if tab_j[col, ligne] != 0:    
        ...
        if (tab_j[col, ligne] == "d"):        
            ...
            nb_cases_vues = nb_cases_vues - 1     
        # Affichage du fond
        can.create_rectangle((col-1)*dim+gap+3, (ligne-1)*dim+gap+3, col*dim+gap-3, ligne*dim+gap-3, width=0, fill="seashell2")
        # Stockage des 0 dans le tableau de jeu   
        tab_j[col, ligne] = 0nb_cases_vues = nb_cases_vues + 1    
    ...
    while nLig < nb_lig: 
      ...
      while nCol <= nb_col:   
        ...
        if tab_m[nCol, nLig] == 9:  
          if tab_j[nCol, nLig] == "?":   
              can.create_image(nCol*dim-dim//2+gap, nLig*dim-dim//2+gap, image = im_mine)             
          elif tab_j[nCol, nLig] == "":                 
              can.create_image(nCol*dim-dim//2+gap, nLig*dim-dim//2+gap, image = im_mine)    
        else:    
          if tab_j[nCol, nLig] == "d":       
              can.create_image(nCol*dim-dim//2+gap, nLig*dim-dim//2+gap, image = im_erreur)
        nCol = nCol+1
     can.create_text((nb_col/2)*dim-15+gap, (nb_lig/2)*dim-5+gap, text='Perdu !', fill='black', font='Arial 50') 
     fen.update_idletasks() # Raffraichit la fenêtre avant le bruitage   
     winsound.PlaySound('explosion.wav', winsound.SND_FILENAME)

def gagne():  
    can.create_text((nb_col/2)*dim-15+gap, (nb_lig/2)*dim-5+gap, text='Bravo !', fill='black', font='Arial 50')  
...
def pointeurG(event):  
    ...
    if on_joue :  
        ...
        if tab_j[nCol, nLig]=="":        
            # Affiche le drapeau        
            can.create_image(nCol*dim-dim//2+gap, nLig*dim-dim//2+gap, image = im_flag) 
            tab_j[nCol, nLig]="d"      
            nb_cases_vues = nb_cases_vues + 1   
            nb_mines_cachees = nb_mines_cachees - 1    
        # si la cellule contient  un drapeau  
            ...
            # Remise à blanc    
            can.create_rectangle((nCol-1)*dim+gap+3,(nLig-1)*dim+gap+3, nCol*dim+gap-3,nLig*dim+gap-3,width=0, fill="grey")    
            # Affiche le ?       
            can.create_text(nCol*dim-dim//2+gap, nLig*dim-dim//2+gap, text="?", fill='black',font='Arial 20')   
            tab_j[nCol, nLig] = "?"  
...
# Après avoir tout déclaré, il est temps de démarrer le programme

fen=Tk() 
fen.title("Démineur") 
fen.resizable(width=False, height=False) 
# Déclarations des variables lorsqu'on ouvre la fenêtre principale
# Initialisation du niveau débutant par défaut
